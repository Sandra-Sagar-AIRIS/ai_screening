"""forward normalize rbac schema

Revision ID: fe22a3555441
Revises: a1f676743968
Create Date: 2026-04-30 15:43:37.329388
"""
...
revision: str = 'fe22a3555441'
down_revision: str | None = 'a1f676743968'
...
def upgrade() -> None:
    ...
    # 1) permissions catalog enrichment for DB-driven UI
    if "permissions" in tables:
        permission_cols = {c["name"] for c in inspector.get_columns("permissions", schema=schema)}
        if "module" not in permission_cols:
            op.add_column("permissions", sa.Column("module", sa.String(length=128), nullable=True), schema=schema)
        if "display_name" not in permission_cols:
    ...
    # Seed default role records for every organization.
    op.execute(
        sa.text(
            f"""
    ...
    # Refresh table/column cache after possible DDL.
    inspector = inspect(bind)
    tables = set(inspector.get_table_names(schema=schema))

    # 3) role_permissions.role -> role_id
    if "role_permissions" in tables:
        ...
        if "role_id" not in rp_cols:
            ...
            # If still unresolved, remap to recruiter to keep data operational.
            op.execute(
                sa.text(
                    f"""
                    UPDATE {schema_prefix}role_permissions AS rp
    ...
    # 4) profiles.role_id (keep profiles.role as denormalized key)
    inspector = inspect(bind)
    profile_cols = {c["name"] for c in inspector.get_columns("profiles", schema=schema)}
    if "role_id" not in profile_cols:
